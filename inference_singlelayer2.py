...
def pyramidOnlyDetect(image, savefile):
    ...
    for i in np.arange(0, 100, 5):
        ...
        for c in range(len(mask[0, 0])):
            if mask[:, :, c].sum()>5 and (mask[:, :, c].sum() / len(mask[:, :, c]) < 10):
               # MASK = MASK + mask[:, :, c] * (c + 1) * 1
                MASK = MASK + 0.01*(mask[:, :, c]>0)
                #MASK = mask[:,:,c]
                #plt.imshow(MASK)
                #plt.show()
        allMASK = allMASK + 0.01*np.array(Image.fromarray(MASK).resize([image0.shape[1], image0.shape[0]]))
        #allMASK = allMASK>0
...
def pyramidDetectXZ(image, savefile):
    allMASK = np.zeros([image.shape[0], image.shape[1]]).astype(np.uint8)
    image0 = image
    for i in np.arange(0, 500, 10):
        ...
        for c in range(len(mask[0, 0])):
            if mask[:, :, c].sum()>5 and (mask[:, :, c].sum() / len(mask[:, :, c]) < 10):
               # MASK = MASK + mask[:, :, c] * (c + 1) * 1
                MASK = MASK + (mask[:, :, c])*0.5
                #MASK = mask[:,:,c]
                #plt.imshow(MASK)
                #plt.show()
        tmp = np.array(Image.fromarray(MASK).resize([image0.shape[1], image0.shape[0]]))
        ii, jj = np.where(tmp > 0)
        allMASK[ii,jj] = i
        #allMASK = allMASK>0
        # visualize.display_instances(image, r['rois'], r['masks'], r['class_ids'],
        #                   dataset.class_names, r['scores'], ax=ax,
...
path = "G:\\SLAC\\201907_slac_LIBNet\\ESRF machine learning\\1A_XZ_\\"
savepath = "G:\\SLAC\\201907_slac_LIBNet\\ESRF machine learning\\mask_1A_XZ\\"
valid_images = [".jpg"]
...
# %%
# image = skimage.io.imread("G:\\SLAC\\201907_slac_LIBNet\\ESRF machine learning\\1A_XZ_\\slice_0138.jpg")
# savefile = "C:\\Users\\Jizhou Li\\Desktop\\test.tif"
# image = skimage.color.gray2rgb(image)
# pyramidOnlyDetect(image, savefile)
#
# image = skimage.io.imread("G:\\SLAC\\201907_slac_LIBNet\\ESRF machine learning\\LithB_Proj1_NMC811_sample1A_100nm_fasttomo_2.to-byte_512\\slice_0139.jpg")
# savefile = "C:\\Users\\Jizhou Li\\Desktop\\test.tif"
# image = skimage.color.gray2rgb(image)
# pyramidDetect(image, savefile)
